...
class ControllerKLCWoodbury:
    ...
    def __init__(self, goal, mode):
        ...
        Prob0 = np.zeros((self.zdiscr[0]**2, self.zdiscr[0]**2))

        for i in range(self.zdiscr[0]):
            for j in range(self.zdiscr[0]):
                pf = self.passive_dynamics0[i,j]
                ind1 = i*self.zdiscr[0] + j
                Prob0[ind1] = self.unravelPF(pf)
        ...
        self.z = self.woodbutyMethod(Prob, diagMinusQ, self.zdiscr[0]**2) 
    ...
    def woodbutyMethod(self, P0, Q, dim):
        L0 = Q-P0
        D0 = np.linalg.inv(L0)
